...
export default function AddressesPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setAddresses([]);
      localStorage.setItem(`africart-addresses-${user.email}`, JSON.stringify([]));
    }
  }, [user, router]);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 16 }}>
        {!isAdding ? (
          <>
            {addresses.map((addr, i) => (
              <div key={addr.id} className={`animate-fade-in-up stagger-${i + 1}`} style={{
                ...
                <p style={{ color: 'var(--on-surface-variant)', fontSize: 14, marginBottom: 16 }}>{addr.city}</p>
          ...
          <form onSubmit={handleSave} className="animate-fade-in-up" style={{ background: 'var(--surface)', padding: 16, borderRadius: 12, border: '1px solid var(--outline)', marginTop: 16 }}>
            ...
            <input required placeholder="Name (e.g. Work, Home)" value={addressForm.name} onChange={e => setAddressForm({...addressForm, name: e.target.value})} style={{ width: '100%', padding: 12, background: 'var(--surface-container)', border: '1px solid var(--outline)', borderRadius: 8, color: 'var(--foreground)', marginBottom: 12 }} />
